      ...
      <div class="calendar">
        ...
        <div class="week" v-for="i in 6" :key="i">
          <div class="day" v-for="j in 7" :data-date="calendarMonth[(i-1)*7+j-1].date"
            :class="{
                a_class: calendarMonth[(i-1)*7+j-1].class_code == 0 && calendarMonth[(i-1)*7+j-1].month == calendar.month,
                b_class: calendarMonth[(i-1)*7+j-1].class_code == 1 && calendarMonth[(i-1)*7+j-1].month == calendar.month,
                c_class: calendarMonth[(i-1)*7+j-1].class_code == 2 && calendarMonth[(i-1)*7+j-1].month == calendar.month,
          ...
          >
            <span style="transform:translateY(15px)">
                {{calendarMonth[(i-1)*7+j-1].class_name}}
            </span>
          </div>
        </div>
      </div>
...
export default {
    ...
    methods:{
        ...
        DateDiff(Date1, Date2) { // date object 日期格式
    ...
    computed:{
        ...
        calendarMonth(){
            ...
            for(let i=0;i<42;i++){
                ...
                let class_code = this.DateDiff(new Date(this.baseday),date) % 3
                let class_name = this.class[class_code];
                data.push({
                    year:date.getFullYear(),
                    month:date.getMonth(),
                    date:date.getDate(),
...
<style scoped>
    ...
    .b_class{
        background-color: rgba(255, 223, 78, 0.7);
    }
    .c_class{
        background-color: rgba(52, 154, 223, 0.7);
    }
</style>
